...
export default function HomePage() {
  ...
  const approveTask = async (taskId: string) => {
    if (!sessionId) return;
    await fetch(`${API_URL}/api/tasks/${taskId}/approve`, {
      method: "POST",
  ...
  const retryWithApproval = async (taskId: string, mode: string) => {
    if (!sessionId) return;
    await fetch(`${API_URL}/api/tasks/${taskId}/retry_with_approval`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ session_id: sessionId, task_id: taskId, approval_mode: mode }),
    });
  };
        ...
        {plan.length > 0 && (
          <div className="w-80 shrink-0 border-l overflow-y-auto scrollbar-thin" style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-surface)" }}>
            ...
            <div className="p-3 space-y-3">
              {/* 需监控任务(需人工/长任务) */}
              {plan.filter(t => t.requires_monitor).length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 text-[11px] mb-1.5" style={{ color: "var(--color-text-muted)" }}>
                    <span>需监控</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ backgroundColor: "rgba(234,179,8,0.15)", color: "#fde047" }}>
                      {plan.filter(t => t.requires_monitor && t.status === "completed").length}/{plan.filter(t => t.requires_monitor).length}
                    </span>
                  </div>
                  <div className="space-y-2">
                    {plan.filter(t => t.requires_monitor).map(task => (
                      <TaskCard key={task.task_id} task={task} onApprove={approveTask} onRetry={retryWithApproval} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {plan.filter(t => !t.requires_monitor).length > 0 && (
                <div>
                  <div className="text-[11px] mb-1.5" style={{ color: "var(--color-text-muted)" }}>普通</div>
                  <div className="space-y-2">
                    {plan.filter(t => !t.requires_monitor).map(task => (
                      <TaskCard key={task.task_id} task={task} onApprove={approveTask} onRetry={retryWithApproval} />
                    ))}
...
function TaskCard({ task, onApprove, onRetry }: {
  task: SubTask;
  onApprove?: (taskId: string) => void;
  onRetry?: (taskId: string, mode: string) => void;
}) {
  const colors: Record<string, { bg: string; border: string; dot: string }> = {
    ...
    waiting_approval: { bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.4)", dot: "var(--color-error)" },
  ...
  const isMonitor = task.requires_monitor;
    ...
    <div className="p-3 rounded-lg text-xs" style={{ border: `1px solid ${c.border}`, backgroundColor: c.bg }}>
      <div className="flex items-center justify-between mb-1.5">
        <span className="flex items-center gap-1">
          ...
          {isMonitor && task.approval_mode && (
            <span className="px-1.5 py-0.5 rounded text-[10px] font-medium" style={approvalBadge(task.approval_mode)}>
              {task.approval_mode === "ask" ? "ask 需审批" : task.approval_mode === "auto" ? "auto 自动" : "full 绕过"}
            </span>
          )}
        </span>
      ...
      {/* 需监控任务:进度条 */}
      {isMonitor && task.status === "running" && (
        <div className="mt-2">
          <div className="h-1 rounded-full overflow-hidden" style={{ backgroundColor: "var(--color-surface-alt)" }}>
            <div className="h-full rounded-full" style={{ width: "55%", backgroundColor: "var(--color-warn)" }} />
          </div>
          {task.progress && (
            <div className="mt-1 text-[10px] truncate" style={{ color: "var(--color-text-muted)" }} title={task.progress}>
              {task.progress}
            </div>
          )}
        </div>
      ...
      {task.status === "waiting_approval" && (
        <div className="mt-2 px-2 py-1.5 rounded text-[11px]" style={{ backgroundColor: "rgba(239,68,68,0.15)", color: "var(--color-error)" }}>
          <div className="font-medium">⚠ codex 请求权限</div>
          <div className="mt-0.5 text-[10px] opacity-80">
            {task.approval_mode === "ask"
              ? "ask 策略受限操作过多,建议改为 full/auto 后重试,否则 codex 下一步仍会卡在审批。"
              : "auto 策略下 codex 请求人工确认,可批准继续或改策略重试。"}
          </div>
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {task.approval_mode !== "ask" && (
              <button onClick={() => onApprove?.(task.task_id)} className="px-2 py-0.5 rounded text-[10px] font-medium" style={{ backgroundColor: "var(--color-success)", color: "#000" }}>
                批准继续
              </button>
            )}
            <button onClick={() => onRetry?.(task.task_id, "full")} className="px-2 py-0.5 rounded text-[10px] font-medium border" style={{ borderColor: "var(--color-error)", color: "var(--color-error)" }}>
              改为 full 重试
            </button>
            <button onClick={() => onRetry?.(task.task_id, "auto")} className="px-2 py-0.5 rounded text-[10px] font-medium border" style={{ borderColor: "var(--color-border)", color: "var(--color-text-dim)" }}>
              改为 auto 重试
            </button>
          </div>
        </div>
      ...
      {task.error && task.status !== "waiting_approval" && (
...
function approvalBadge(mode: string) {
  const map: Record<string, { background: string; color: string }> = {
    ask: { background: "rgba(239,68,68,0.2)", color: "#fca5a5" },
    auto: { background: "rgba(234,179,8,0.15)", color: "#fde047" },
    full: { background: "rgba(107,114,128,0.15)", color: "#9ca3af" },
  };
  const s = map[mode] || map.auto;
...
function statusLabel(s: string) {
  const map: Record<string, string> = { pending: "等待", ready: "就绪", running: "执行中", waiting_approval: "待审批", completed: "完成", failed: "失败", canceled: "已取消" };
  return map[s] || s;
}
...
function badgeStyle(t: string) {
  const map: Record<string, { background: string; color: string }> = {
    ...
    codex_cli: { background: "rgba(168,85,247,0.2)", color: "#e9d5ff" },
    retrieval: { background: "rgba(16,185,129,0.15)", color: "#6ee7b7" },
